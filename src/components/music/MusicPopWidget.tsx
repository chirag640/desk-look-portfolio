...
export const MusicPopWidget: React.FC = () => {
  ...
  }, [commandTrigger, currentPlaylist.youtubePlaylistId, setHasLoadedPlayer]);
  ...
  return (
    <>
      {/* ── 1. MINIMIZED FLOATING VINYL DISC BADGE ── */}
      {isMinimized && (
        <div
          data-music-popup="true"
          onClick={() => {
            playClick();
            useWindowManager.getState().openWindow("music");
          }}
          className="fixed bottom-20 right-6 z-50 flex items-center gap-3 px-3.5 py-2 rounded-full bg-[#131926]/95 border border-white/20 shadow-2xl backdrop-blur-xl cursor-pointer group hover:scale-105 transition-all"
        ...
        >
          ...
          {/* Quick Play/Pause Button on Mini Badge */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              handleTogglePlay();
      ...
      {/* ── 2. FULL EXPANDED VINYL PLAYER WINDOW ── */}
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => {
                  ...
                  postYTCommand("pauseVideo");
                  setIsPlaying(false);
                  useWindowManager.getState().closeWindow("music");
                }}
              ...
              <button
                onClick={() => {
                  playClick();
                  useWindowManager.getState().minimizeWindow("music");
                }}
      ...
      {/* ── 3. PERSISTENT BACKGROUND YOUTUBE HOST CONTAINER ── */}
      {/* 
        This iframe only mounts after the user starts playback (hasLoadedPlayer = true),
        preventing any auto-play on initial portfolio load.
        Once mounted, it stays permanently alive even if the user minimizes or closes the window,
        so music continues playing without being unmounted or reset by React.
      */}
      {hasLoadedPlayer && (
        <div
          aria-hidden={!showVideoDrawer}
          className={`fixed transition-all duration-300 ${
            showVideoDrawer && isPlayerOpen && !isMinimized
              ? "bottom-[84px] right-4 sm:right-8 w-[320px] sm:w-[350px] h-[185px] z-50 pointer-events-auto opacity-100 shadow-2xl"
              : "bottom-4 right-4 w-[280px] h-[160px] pointer-events-none opacity-[0.001] -z-50"
          }`}
        >
          <iframe
            ref={iframeRef}
